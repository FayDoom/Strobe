...
class Terramodis(Connector):
	...
	def get_image_info(self):
		...
		img_url_template = self.images_url.format(
			...
			TileMatrixSet="250m",
			ZoomLevel=self.sources[self.source_name]["zoom_level"],
			TileRow="{TileRow}",
			TileCol="{TileCol}",
		)


		tile_x = math.ceil((Utils.get_screen_size()["width"]+self.sources[self.source_name]["offset_x"])/self.tile_size["width"])
		tile_y = math.ceil((Utils.get_screen_size()["height"]+self.sources[self.source_name]["offset_y"])/self.tile_size["height"])
			...
			for x in range(tile_x):
				url_tab[y].append(
					img_url_template.format(
						TileCol=x+self.sources[self.source_name]["start_tile_x"],
						TileRow=y+self.sources[self.source_name]["start_tile_y"],
					)
				)
